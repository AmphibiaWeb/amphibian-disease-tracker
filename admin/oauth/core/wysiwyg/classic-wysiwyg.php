...
function parseBlock($block, $sanitize = true, $strip = true, $paragraph = true)
{
  ...
  $raise_error = false;
    ...
    if (!is_string($block)) {
        return array(false,'error' => 'A string wasn\'t provided.');
    }
    if ($sanitize) {
  ...
  ***/
  ...
  if (strpos($parsed, '[img:') !== false) {
      ...
      while ($pos !== false) {
          ...
          $img_e = explode(',', $img);
          if (sizeof($img_e < 2)) {
              return array(false,'error' => 'Fatal Error: Bad Image Syntax');
          }
...
// The tag is self-closed, so fancy parsing isn't needed here.
  ...
  if (!$strip) {
    ...
    if ($raise_error) {
        return array(true,'html' => $parsed,'error_log' => $error_log,'new_edit' => deparseBlock($parsed));
    }
